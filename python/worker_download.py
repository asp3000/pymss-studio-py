...
def _resolve_aria2c_path() -> str | None:
    search_dirs = list(os.get_exec_path())
    # Bundled aria2c/ffmpeg live in PYMSS_STUDIO_BIN (injected by the Python GUI
    # bridge). Check it first so the portable build works without a system aria2.
    bin_dir = os.environ.get("PYMSS_STUDIO_BIN")
    if bin_dir and bin_dir not in search_dirs:
        search_dirs.insert(0, bin_dir)
    seen: set[str] = set()
